...
class BonusCalculate extends Model {
    ...
    public function getCalculateResult($game, $eventType) {
        ...
        if($game['play_type'] == 'outright') {
            $outrightInfo = Loader::model('SportsBasketballOutright')->find($game['game_id']);
            if(!$outrightInfo['sbo_result']) {
                return false;
            }
            $result       = $outrightInfo['sbo_result'];
            $odds         = json_decode($outrightInfo['sbo_odds'], true);
            $game['team'] = $odds[$game['odds_key']]['team'];
        } else {
            //获取主盘口id
            if (!$game['master_game_id']) {
                $masterGameId = Loader::model('Games', 'basketball')->getMasterGameIdByGameId($game['game_id']);
                if(!$masterGameId) {
                    return false;
                }
            ...
            //2. 通过对阵id和盘口类型获取赛果
            $result = Loader::model('SportsBasketballResults')->find($masterGameId);
            if(!$result) {
                return false;
            }
        }
        ...
        //计算赛果
        $game['calculate_result'] = Loader::model('ResultCalculate', 'basketball')->calculate($game, $result, $eventType);
