...
class Outright extends Model {
    ...
    public function getInfoByGameId($id, $field = '', $isCache = false) {
        $cacheKey = Config::get('cache_option.prefix')['sports_common'] . 'outright:football_outright_info_'  . md5($id  . $field);
        if ($isCache) {
        ...
        $info = Loader::model('SportsFootballOutright')->field($field)->find($id);
        if (!$info) {
            return false;
        }
        $info = $info->toArray();
