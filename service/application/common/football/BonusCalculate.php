...
class BonusCalculate extends Model {
    ...
    public function getCalculateResult($game, $eventType) {
        ...
        if($game['play_type'] == 'outright') {
            $outrightInfo = Loader::model('SportsFootballOutright')->find($game['game_id']);
            if(!$outrightInfo['sfo_result']) {
                return false;
            }
            $result       = $outrightInfo['sfo_result'];
            $odds         = json_decode($outrightInfo['sfo_odds'], true);
            $game['team'] = $odds[$game['odds_key']]['team'];
        } else {
            //获取主盘口id
            if (!$game['master_game_id']) {
                $masterGameId = Loader::model('Games', 'football')->getMasterGameIdByGameId($game['game_id']);
                if(!$masterGameId) {
                    return false;
                }
            ...
            //2. 通过对阵id和盘口类型获取赛果
            $result = Loader::model('SportsFootballResults')->find($masterGameId);
            if(!$result) {
                return false;
            }
        }
        ...
        //计算赛果
        $game['calculate_result'] = Loader::model('ResultCalculate', 'football')->calculate($game, $result, $eventType);
