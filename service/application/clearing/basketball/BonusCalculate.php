...
class BonusCalculate extends \app\common\logic\BonusCalculate {
    ...
    public function calculateSchedules() {
        //获取比赛结束并且未算奖的对阵
        $schedulesInfo = Loader::model('Schedules', 'basketball')->getNoClearing();
        if(!$schedulesInfo) {
            return false;
        }
        ...
        foreach($schedulesInfo as $scheduleInfo) {
            $scheduleLockKey = $scheduleLockPrefix . $scheduleInfo->sbs_id;
        ...
        foreach($schedulesInfo as $scheduleInfo) {
            //判断对阵是否有赛果
            $resultCount = Loader::model('SportsBasketballResults')->where(['sbr_game_id' => $scheduleInfo->sbs_master_game_id])->count();
            if (!$resultCount) {
                Cache::rm($scheduleLockPrefix . $scheduleInfo->sbs_id);
                continue;
            }
            ...
            $where[] = ['exp', 'FIND_IN_SET(' . $scheduleInfo->sbs_id . ',so_source_ids)'];
            ...
            if ($orderCount > 0) {
                ...
                //比赛结束才能改为已结算
                if ($scheduleInfo->sbs_status == Config::get('status.basketball_schedule_status')['game_over']) {
                    Loader::model('common/Schedules', 'basketball')->updateClearingStatusById($scheduleInfo->sbs_id, Config::get('status.basketball_schedule_clearing')['yes']);
                }
            }
    ...
    public function calculateOutright() {
        //返回已出赛果并且未算奖的冠军盘口id
        $gameIds = Loader::model('Outright', 'basketball')->getNoClearing();
        if(!$gameIds) {
            return false;
        }
        ...
        //给这些冠军盘口加正在算奖锁;
        $cacheTag           = md5(__CLASS__ . __FUNCTION__);
        $outrightLockPrefix = Config::get('cache_option.prefix')['sports_basketball_outright_lock'];
        $orderLockPrefix    = Config::get('cache_option.prefix')['sports_order_lock'];
        ...
        foreach($gameIds as $key => $gameId) {
            ...
            if ($orderCount > 0) {
                ...
                //该对阵没有未结算订单直接修改状态为已结算
                Loader::model('Outright', 'basketball')->updateOutrightClearing($gameId);
            }
