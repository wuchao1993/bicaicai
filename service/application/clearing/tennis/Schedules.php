...
class Schedules extends \app\common\tennis\Schedules {
    /**
     * 撤销人工标记为撤单的单子
     * @return bool
     */
    public function cancelMarkedOrders() {
        $where = [
            'sts_check_status' => Config::get('status.tennis_schedule_check_status')['wait_cancel'],
            'sts_clearing' => Config::get('status.tennis_schedule_clearing')['no'],
        ];
        $schedules = Loader::model('SportsTennisSchedules')
            ->where($where)
            ->field('sts_id,sts_master_game_id')
            ->select();
        if (!$schedules) {
            return false;
        }
        ...
        foreach($schedules as $schedule) {
            $scheduleLockKey = $scheduleLockPrefix . $schedule->sts_id;
        ...
        foreach($schedules as $schedule) {
            ...
            $where = array_merge($whereCommon, [['exp', 'FIND_IN_SET(' . $schedule->sts_id . ',so_source_ids)']]);
                    ...
                    foreach($orders as $order) {
                        if ($order->so_status == Config::get('status.order_status')['distribute']) {
                            if ($order->so_event_type == Config::get('status.order_event_type')['parlay']) {
                                Loader::model('common/Orders', 'logic')->handleDistributedReturn($order->so_no, $schedule->sts_master_game_id);
                                Loader::model('common/Orders', 'logic')->handleCancel($order->so_no, '人工撤单', 'hand_cancel', $schedule->sts_master_game_id);
                            } else {
            ...
            $whereCommon['so_source_ids'] = $schedule->sts_id;
            $orderCount = Loader::model('SportsOrders')->where($whereCommon)->count();
            if (!$orderCount) {
                //改为结算状态
                $this->updateClearingStatusById($schedule->sts_id, Config::get('status.tennis_schedule_clearing')['yes']);

                //修改为已撤单状态
                $this->updateCheckStatusById($schedule->sts_id, Config::get('status.tennis_schedule_check_status')['canceled']);
            }
        }
    ...
    public function cancelClearingMarkedOrders() {
        $where = [
            'sts_check_status' => Config::get('status.tennis_schedule_check_status')['wait_cancel_clearing'],
            //'sts_clearing' => Config::get('status.tennis_schedule_clearing')['yes'],
        ];
        $schedules = Loader::model('SportsTennisSchedules')
            ->where($where)
            ->field('sts_id,sts_master_game_id')
        ...
        foreach($schedules as $schedule) {
            ...
            $where[] = ['exp', 'FIND_IN_SET(' . $schedule->sts_id . ',so_source_ids)'];
            ...
            do {
                ...
                if($orders) {
                    ...
                    foreach($orders as $orderNo) {
                        //TODO logo
                        Loader::model('common/Orders', 'logic')->handleDistributedReturn($orderNo, $schedule->sts_master_game_id);
            ...
            if (!$orderCount) {
                //改为未结算状态
                $this->updateClearingStatusById($schedule->sts_id, Config::get('status.tennis_schedule_clearing')['no']);

                //修改状态为等待人工结算
                $this->updateCheckStatusById($schedule->sts_id, Config::get('status.tennis_schedule_check_status')['wait_hand_clearing']);
            }
        }
    ...
    public function clearMarkedOrders() {
        $where = [
            'sts_check_status' => Config::get('status.tennis_schedule_check_status')['wait_clearing'],
            'sts_clearing' => Config::get('status.tennis_schedule_clearing')['no'],
        ];
        $scheduleIds = Loader::model('SportsTennisSchedules')
            ->where($where)
            ->column('sts_id');
        if (!$scheduleIds) {
            return false;
        ...
        foreach($scheduleIds as $scheduleId) {
            ...
            if (!$orderCount) {
                //改为结算状态
                $this->updateClearingStatusById($scheduleId, Config::get('status.tennis_schedule_clearing')['yes']);

                //修改状态为等待人工结算
                $this->updateCheckStatusById($scheduleId, Config::get('status.tennis_schedule_check_status')['normal']);
            }
        }
    ...
    public function getNoClearing() {
        $where = [
            ...
            'sts_status' => ['IN', [
                ...
                Config::get('status.tennis_schedule_status')['game_over'],
            ]],
        ];
        $ret = Loader::model('SportsTennisSchedules')
            ->where($where)
            ->field('sts_id,sts_master_game_id,sts_status')
            ->select();
