...
class BonusCalculate extends \app\common\logic\BonusCalculate {
    ...
    public function calculateSchedules() {
        //获取比赛结束并且未算奖的对阵
        $schedulesInfo = Loader::model('Schedules', 'football')->getNoClearing();
        if(!$schedulesInfo) {
            return false;
        }

        //给这些对阵加正在算奖锁; 算奖的时候后台不能做修改比分等操作
        ...
        foreach($schedulesInfo as $scheduleInfo) {
            $scheduleLockKey = $scheduleLockPrefix . $scheduleInfo->sfs_id;
        ...
        foreach($schedulesInfo as $key => $scheduleInfo) {
            //判断对阵是否有赛果
            $resultCount = Loader::model('SportsFootballResults')->where(['sfr_game_id' => $scheduleInfo->sfs_master_game_id])->count();
            if (!$resultCount) {
                Cache::rm($scheduleLockPrefix . $scheduleInfo->sfs_id);
                continue;
            }
            ...
            $where[] = ['exp', 'FIND_IN_SET(' . $scheduleInfo->sfs_id . ',so_source_ids)'];
            ...
            if ($orderCount > 0) {
                ...
                //比赛结束才能改为已结算，因为足球有半场结算
                if ($scheduleInfo->sfs_status == Config::get('status.football_schedule_status')['game_over']) {
                    Loader::model('common/Schedules', 'football')->updateClearingStatusById($scheduleInfo->sfs_id, Config::get('status.football_schedule_clearing')['yes']);
                }
            }
    ...
    public function calculateOutright() {
        //返回已出赛果并且未算奖的冠军盘口id
        $gameIds = Loader::model('Outright', 'football')->getNoClearing();
        if(!$gameIds) {
            return false;
        }
        ...
        //给这些冠军盘口加正在算奖锁;
        $cacheTag           = md5(__CLASS__ . __FUNCTION__);
        $outrightLockPrefix = Config::get('cache_option.prefix')['sports_football_outright_lock'];
        $orderLockPrefix    = Config::get('cache_option.prefix')['sports_order_lock'];
        ...
        foreach($gameIds as $key => $gameId) {
            ...
            if ($orderCount > 0) {
                ...
                //该对阵没有未结算订单直接修改状态为已结算
                Loader::model('Outright', 'football')->updateOutrightClearing($gameId);
            }
